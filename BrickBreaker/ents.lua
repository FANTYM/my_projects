...
function ents.think(updateDelta)
	...
	for k, ent in pairs(ents.entList) do 
		if not (ent == nil) then
			...
			if ent.pos.x - (ent.anims[0].fSize.x * 0.5) < 0 then
				ent.vel.x = ent.vel.x + ((ent.pos.x - (ent.anims[0].fSize.x * 0.5)) * -1)
			end
			
			if ent.pos.x + (ent.anims[0].fSize.x * 0.5) > screenSize.x then
				ent.vel.x = ent.vel.x - ((ent.pos.x + (ent.anims[0].fSize.x * 0.5)) - screenSize.x)
			end
